...
You can cap how many subagents a single request may launch using the `subagent_limit` option.
...
Unset means unlimited. Set it to `0` to disable subagents entirely. Each subagent runs its own model context, so this cap controls how much one request can spend on delegation. Resuming an existing subagent with `task_id` does not count against the limit.
